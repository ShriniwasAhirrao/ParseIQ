...
class TestIntegrationMetadataEnrichmentAgent(unittest.TestCase):
    ...
    def test_run_pipeline_full_flow(self, mock_llm_enricher, mock_metadata_extractor, mock_file_loader, mock_get_api_key):
        # Arrange — load_file must return dict of tables, not a plain list
        _table_meta = {
            "table_metadata": {"attributes": {}, "data_quality_score": 90,
                               "anomaly_summary": {"total_anomalies": 0}, "top_issues": [],
                               "dataset_info": {"total_records": 1},
                               "data_profiling": {"duplicate_analysis": {}, "record_completeness": {}}},
            "summary": {"total_records": 1}
        }
        mock_file_loader.return_value.load_file.return_value = {"main_table": [{"field1": "value1"}]}
        mock_metadata_extractor.return_value.extract_metadata.return_value = _table_meta
        mock_llm_enricher.return_value.enrich_metadata.return_value = {
            "overall_assessment": {"quality_grade": "A", "overall_score": 95}
        }
