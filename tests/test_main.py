...
class TestMetadataEnrichmentAgent(unittest.TestCase):
    ...
    def test_run_pipeline_skip_llm(self):
        # load_file must return a dict of {table_name: [records]} — not a plain list
        mock_tables = {"main_table": [{"a": 1}]}
        with patch.object(self.agent.file_loader, 'load_file', return_value=mock_tables), \
             patch.object(self.agent.metadata_extractor, 'extract_metadata',
                          return_value={"table_metadata": {"attributes": {}, "data_quality_score": 100,
