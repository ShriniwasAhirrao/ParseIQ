...
class LLMEnricher:
    def __init__(self, config: Dict[str, Any]):
            ...
            self.config = config
            self.api_key = config.get('api_key') or None  # validated lazily at call time
            self.site_url = config.get('site_url', 'http://localhost:3000')
            self.site_url = config.get('site_url', 'http://localhost:3000')
            self.site_name = config.get('site_name', 'ParseIQ')
            self.base_url = config.get('base_url', 'https://openrouter.ai/api/v1')
            self.model = config.get('model', 'mistralai/mistral-small-3.1-24b-instruct:free')
            self.max_tokens = config.get('max_tokens', 4000)
            self.temperature = config.get('temperature', 0.1)
            self.debug = config.get('debug', False)
            self.current_date = datetime.now().date()
            
# CONSERVATIVE RATE LIMIT for FREE TIER (10 RPM burst)
            self._last_test_time = 0
            self._test_interval = 300  # 5 minutes debounce
            self._tokens = 10       # Start with 10 tokens
            self._max_tokens = 10   # Bucket capacity = 10 RPM
            self._last_refill = time.time()
            self._refill_rate = 10 / 60.0  # 10 tokens per minute
    ...
    def _setup_logging(self) -> logging.Logger:
        """LLMEnricher writes to its own debug log file only.
        Console output is handled exclusively by the root logger in main.py.
        propagate=False prevents double-printing to console.
        """
        logger = logging.getLogger('LLMEnricher')
        logger.setLevel(logging.DEBUG if self.debug else logging.INFO)

        # Remove ALL existing handlers cleanly to prevent duplicates across re-runs
        ...
        # Do NOT propagate — root logger already handles console; propagation = duplicates
        logger.propagate = False

        self.logger = logger
        logger.info(f"LLM debug log: {log_file}")
        return logger
        
    def _load_prompt_template(self) -> str:
        """Load the prompt template from file."""
        import os as _os
        _default = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), 'prompt_template.txt')
        template_path = self.config.get('prompt_template_path', _default)
        
        try:
            with open(template_path, 'r', encoding='utf-8') as f:
                template = f.read()
            
            # Escape literal curly braces in JSON example block
            # Find the ```json block and escape its braces
            import re
            def escape_json_examples(template):
                # Escape {{ and }} outside of our real placeholders
                parts = re.split(r'(\{metadata_summary\}|\{current_date\})', template)
                escaped = []
                for part in parts:
                    if part in ('{metadata_summary}', '{current_date}'):
                        escaped.append(part)
                    else:
    ...
    def _detect_table_structure(self, metadata_summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'tables' in metadata_summary and isinstance(metadata_summary['tables'], dict):
            ...
            def _rc(t):
                # record count may live in different places depending on who built the dict
                return (t.get('record_count') or
                        t.get('summary', {}).get('total_records') or
                        t.get('table_metadata', {}).get('dataset_info', {}).get('total_records', 0))
            table_info["total_records"] = sum(_rc(t) for t in metadata_summary['tables'].values())
        else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 400:
                    ...
                    self.logger.error(f"❌ 400 Bad Request for model '{self.model}': {err_body}")
                    if self.debug:
                        print(f"❌ 400 error: {err_body}")
                    raise Exception(f"400 Bad Request: {err_body}")
            ...
            if attempt < max_retries - 1:
                self.logger.info(f"⏳ Backing off {delay:.1f}s")
                time.sleep(delay)

        raise Exception(f"Max retries exhausted for model: {self.model}")
    
    def _consume_token(self) -> bool:
        """Simple token bucket: 10 tokens/min (conservative for free tier)"""
        now = time.time()
        ...
        if self._tokens >= 1:
            ...
            self.logger.warning("🏪 Token bucket empty - rate limited locally")
            return False
    
    def _jitter(self, max_jitter=1.0) -> float:
    ...
    def enrich_metadata(self, metadata_summary: Dict[str, Any], model: str = None) -> Dict[str, Any]:
        ...
        if not isinstance(metadata_summary, dict):
            error_msg = f"Invalid input: metadata_summary is not dict, got {type(metadata_summary)}"
            self.logger.error(error_msg)
            if self.debug:
                print(f"❌ DEBUG: {error_msg}")
            return self._create_fallback_enrichment({}, {}, {}, {})
        
        self.logger.info(f"Input metadata keys: {list(metadata_summary.keys())}")
        ...
        try:
            ...
            # STEP 1: Detect dataset type - WRAPPED
            self.logger.info("STEP 1: Detecting dataset type...")
            try:
                dataset_type = self._detect_dataset_type(metadata_summary)
                self.logger.info(f"Dataset type detected: {dataset_type}")
            except Exception as e:
                self.logger.error(f"Error in _detect_dataset_type: {e}")
                self.logger.error(f"Traceback: {traceback.format_exc()}")
                dataset_type = "single_table"
            
            # STEP 2: Logical validation - WRAPPED
            self.logger.info("STEP 2: Logical validation...")
            try:
                logical_validation = self._perform_comprehensive_logical_validation(metadata_summary)
                self.logger.info(f"Logical validation complete: {len(logical_validation.get('temporal_issues', []))} temporal issues")
            except Exception as e:
                self.logger.error(f"Error in logical validation: {e}")
                self.logger.error(f"Traceback: {traceback.format_exc()}")
                logical_validation = {}
            
            # STEP 3: Quality metrics - WRAPPED  
            ...
            try:
                ...
                self.logger.info(f"Quality grade: {corrected_metrics.get('quality_grade', 'N/A')}")
            except Exception as e:
                self.logger.error(f"Error in quality metrics: {e}")
                self.logger.error(f"Traceback: {traceback.format_exc()}")
                corrected_metrics = {"quality_grade": "Unknown", "corrected_score": 0}
            
            # STEP 4: Cross-table analysis - WRAPPED
            cross_table_analysis = {}
            if dataset_type == "multi_table":
                self.logger.info("STEP 4: Cross-table analysis...")
                try:
                    cross_table_analysis = self._analyze_cross_table_relationships(metadata_summary)
                except Exception as e:
                    self.logger.error(f"Error in cross-table analysis: {e}")
                    self.logger.error(f"Traceback: {traceback.format_exc()}")
            
            # STEP 5: Business rules - WRAPPED
            business_rule_violations = []
            try:
                business_rule_violations = self._validate_business_rules(metadata_summary)
            except Exception as e:
                self.logger.error(f"Error in business rules validation: {e}")
                self.logger.error(f"Traceback: {traceback.format_exc()}")
            
            # STEP 6: Enhanced metadata
            self.logger.info("STEP 6: Building enhanced metadata...")
            ...
            # STEP 7: Prepare prompt metadata - WRAPPED
            self.logger.info("STEP 7: Preparing prompt metadata...")
            try:
                prompt_metadata = enhanced_metadata.copy()
                if dataset_type == "multi_table":
                    tables = prompt_metadata.get('tables', {})
                    for table_name, table_data in tables.items():
                        if 'table_metadata' in table_data and 'table_name' in table_data['table_metadata']:
                            tables[table_name] = table_data['table_metadata']
                    prompt_metadata['tables'] = tables
                

                
                # Use format_map with SafeDict to ignore unknown keys (JSON content inside template)
                class SafeDict(dict):
                    def __missing__(self, key):
                        return '{' + key + '}'

                # Compress metadata to token-efficient summary (~8k chars max)
                prompt_json = self._compress_metadata_for_prompt(prompt_metadata, max_chars=8000)
                self.logger.info(f"Using compressed metadata: {len(prompt_json)} chars")

                formatted_prompt = self.prompt_template.format_map(SafeDict(
                    metadata_summary=prompt_json,
                    current_date=self.current_date.isoformat()
                ))
                self.logger.info("Prompt formatting successful")
            except Exception as e:
                self.logger.error(f"Error in prompt preparation/formatting: {e}")
                self.logger.error(f"Traceback: {traceback.format_exc()}")
            ...
            if self.debug:
                print(f"🔍 DEBUG: Enhanced metadata prepared. Issues: {corrected_metrics.get('total_logical_issues', 0)}")
                print(f"🔍 DEBUG: Dataset type: {dataset_type}")
            
            # STEP 8+: LLM call (existing logic with added logging)
            ...
            if not json_content:
                self.logger.error("Failed to extract JSON - using fallback")
                return self._create_fallback_enrichment(enhanced_metadata, corrected_metrics, logical_validation, cross_table_analysis)
            ...
            try:
                ...
                self.logger.error(f"Content preview: {json_content[:200]}")
                return self._create_fallback_enrichment(enhanced_metadata, corrected_metrics, logical_validation, cross_table_analysis)
            
            # Merge and return (existing)
            ...
            self.logger.info(f"Enrichment complete: {enriched_data.get('overall_assessment', {}).get('quality_grade', '?')}")
            return enriched_data
            
        except Exception as e:
            error_msg = f"Critical error in enrich_metadata: {e}"
            self.logger.error(error_msg)
            self.logger.error(f"Full traceback: {traceback.format_exc()}")
            if self.debug:
                print(f"❌ CRITICAL: {error_msg}")
            return self._create_fallback_enrichment(metadata_summary, {}, {}, {})
    def _run_per_table_enrichment(self, enhanced_metadata: Dict[str, Any],
                                   corrected_metrics: Dict[str, Any],
                                   logical_validation: Dict[str, Any],
                                   cross_table_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """For multi-table datasets: call LLM once per table with a focused prompt,
        then aggregate results into the standard enriched format.

        This prevents the 400/context-limit errors caused by sending all tables at once.
        """
        dataset_overview = enhanced_metadata.get('dataset_overview', {})
        ...
        for table_name in table_names:
            self.logger.info(f"  LLM enriching table: {table_name}")
            if self.debug:
                print(f"  🔍 Enriching table: {table_name}")

            # Build a single-table metadata slice
            table_slice = {
                'dataset_type': 'single_table',
                ...
                'dataset_overview': {
                    'table_summaries': {
                        table_name: table_summaries[table_name]
                    }
                },
                'corrected_quality_metrics': corrected_metrics,
                'logical_validation': logical_validation,
                'cross_table_analysis': {},
                'business_rule_violations': enhanced_metadata.get('business_rule_violations', []),
            }
            ...
            prompt_json = self._compress_metadata_for_prompt(table_slice, max_chars=6000)
            try:
                formatted_prompt = self.prompt_template.format_map(SafeDict(
                    metadata_summary=prompt_json,
                    current_date=self.current_date.isoformat()
                ...
                self.logger.error(f"Prompt formatting error for {table_name}: {e}")
                continue

            try:
                response = self._make_api_request(formatted_prompt)
            except Exception as api_err:
                self.logger.error(f"LLM failed for table {table_name}: {api_err}")
                continue

            json_content = self._extract_json_from_response(response)
            if not json_content:
                self.logger.warning(f"No JSON extracted for table {table_name}")
                continue

            try:
                table_result = json.loads(json_content)
            except json.JSONDecodeError as e:
                self.logger.error(f"JSON parse error for table {table_name}: {e}")
                continue

            per_table_results[table_name] = table_result

            # Aggregate findings
            score = table_result.get('overall_assessment', {}).get('overall_score')
            if score is not None:
                table_quality_scores[table_name] = score

            for issue in table_result.get('critical_issues', []):
                issue.setdefault('table', table_name)
                all_critical_issues.append(issue)

            for rec in table_result.get('recommendations', []):
                rec.setdefault('table', table_name)
                all_recommendations.append(rec)

            for field in table_result.get('field_analysis', []):
                field.setdefault('table_name', table_name)
                all_field_analysis.append(field)

        # Build aggregated enriched result
        if not per_table_results:
            return None  # All tables failed → caller uses fallback

        avg_score = round(sum(table_quality_scores.values()) / len(table_quality_scores), 1) if table_quality_scores else 0
        grade = corrected_metrics.get('quality_grade', 'C')

        # Use first table's overall_assessment structure, override scores with aggregated
        first_result = next(iter(per_table_results.values()))
        aggregated = {
            'dataset_overview': {
                'table_count': len(table_names),
                'table_names': table_names,
                'dataset_type': 'multi_table',
                'cross_table_relationships': cross_table_analysis.get('relationships', []),
                'total_records': sum(
                    table_summaries.get(t, {}).get('record_count', 0) for t in table_names
                ),
                'analysis_timestamp': datetime.now().isoformat(),
            },
            'overall_assessment': {
                **first_result.get('overall_assessment', {}),
                'overall_score': avg_score,
                'quality_grade': grade,
                'table_scores': table_quality_scores,
            },
            'table_analysis': [
                {
                    'table_name': tn,
                    **per_table_results[tn].get('overall_assessment', {}),
                    'table_quality_score': table_quality_scores.get(tn, 0),
                }
                for tn in table_names if tn in per_table_results
            ],
            'cross_table_analysis': cross_table_analysis,
            'critical_issues': all_critical_issues,
            'field_analysis': all_field_analysis,
            'recommendations': all_recommendations,
            'business_rule_violations': first_result.get('business_rule_violations', []),
            'risk_assessment': first_result.get('risk_assessment', {}),
            'governance_recommendations': first_result.get('governance_recommendations', {}),
            'monitoring_suggestions': first_result.get('monitoring_suggestions', []),
            'validation_pipeline_recommendations': first_result.get('validation_pipeline_recommendations', {}),
            'next_steps': first_result.get('next_steps', {}),
            'per_table_details': per_table_results,
        }
        return aggregated

    def _compress_metadata_for_prompt(self, metadata_summary: Dict[str, Any], max_chars: int = 8000) -> str:
        """Produce a compact, issue-focused summary of metadata for the LLM.

        Strategy for large / deeply-nested multi-table datasets (single LLM call):
        1. Dataset header  — table list, record counts, quality grade
        2. Per-table block — completeness %, duplicate count, field count
        3. Field detail    — ONLY fields with issues (null > 5 %, anomalies, outliers, patterns)
                            Clean fields are skipped entirely to save tokens
        4. Validation      — top temporal / cross-field / business-rule issues (capped)
        5. Cross-table     — relationship violations, orphaned records (capped)

        If the result still exceeds max_chars, we progressively drop:
          a) numeric stats from problem fields
          b) field anomaly text (keep counts only)
          c) oldest / lowest-severity issues
        """
        try:
            dataset_overview = metadata_summary.get('dataset_overview', {})
            table_summaries = dataset_overview.get('table_summaries', {})
            corrected = metadata_summary.get('corrected_quality_metrics', {})
            logical_val = metadata_summary.get('logical_validation', {})
            cross_table = metadata_summary.get('cross_table_analysis', {})
            biz_violations = metadata_summary.get('business_rule_violations', [])

            # ── 1. DATASET HEADER ──────────────────────────────────────────────────
            compact: Dict[str, Any] = {
                "dataset_type": metadata_summary.get('dataset_type', 'unknown'),
                "total_tables": len(table_summaries),
                "analysis_date": self.current_date.isoformat(),
                "table_names": list(table_summaries.keys()),
                "quality_grade": corrected.get('quality_grade', 'N/A'),
                "quality_score": round(corrected.get('corrected_score', 0), 1),
                "component_scores": {
                    k: round(v, 1) if isinstance(v, float) else v
                    for k, v in corrected.get('component_scores', {}).items()
                },
                "total_logical_issues": corrected.get('total_logical_issues', 0),
                "tables": {},
            }

            # ── 2. PER-TABLE BLOCKS ────────────────────────────────────────────────
            for table_name, tdata in table_summaries.items():
                if not isinstance(tdata, dict):
                    continue
                field_analysis = tdata.get('field_analysis', {})
                if not isinstance(field_analysis, dict):
                    field_analysis = {}

                # ── 3. FIELD DETAIL (issue-only) ───────────────────────────────────
                problem_fields: Dict[str, Any] = {}
                clean_field_names: List[str] = []

                for field_name, fdata in field_analysis.items():
                    if not isinstance(fdata, dict):
                        continue

                    null_pct = float(fdata.get('null_percentage', fdata.get('null_rate', 0)) or 0)
                    anomalies = fdata.get('anomalies', [])
                    if not isinstance(anomalies, list):
                        anomalies = []
                    outlier_count = fdata.get('outlier_count', 0) or 0
                    pattern_issues = fdata.get('pattern_violations', fdata.get('format_issues', []))
                    if not isinstance(pattern_issues, list):
                        pattern_issues = []
                    ...
                    if has_issue:
                        entry: Dict[str, Any] = {
                            "type": fdata.get('data_type', fdata.get('inferred_type', 'unknown')),
                            "null_pct": round(null_pct, 1),
                        }
                        if fdata.get('unique_percentage') is not None:
                            entry["unique_pct"] = round(float(fdata['unique_percentage']), 1)
                        if anomalies:
                            entry["anomalies"] = [str(a)[:80] for a in anomalies[:3]]
                        if outlier_count:
                            entry["outliers"] = outlier_count
                        if pattern_issues:
                            entry["pattern_issues"] = [str(p)[:60] for p in pattern_issues[:2]]
                        # Numeric range (concise)
                        if fdata.get('data_type') in ('integer', 'float', 'numeric'):
                            for stat in ('min_value', 'max_value', 'mean'):
                                v = fdata.get(stat)
                                if v is not None:
                                    try:
                                        entry[stat.replace('_value', '')] = round(float(v), 2)
                                    except (TypeError, ValueError):
                                        pass
                        problem_fields[field_name] = entry
                    else:
                ...
                table_block: Dict[str, Any] = {
                    ...
                    "completeness_pct": round(float(tdata.get('completeness_rate', 0) or 0), 1),
                    "duplicate_count": tdata.get('duplicate_count', 0),
                    "problem_field_count": len(problem_fields),
                    "clean_fields": clean_field_names,  # names only, no detail
                }
                if problem_fields:
                    table_block["problem_fields"] = problem_fields

                compact["tables"][table_name] = table_block

            # ── 4. VALIDATION ISSUES ───────────────────────────────────────────────
            validation_block: Dict[str, Any] = {}
            temporal = logical_val.get('temporal_issues', [])
            if isinstance(temporal, list) and temporal:
                validation_block["temporal"] = [str(i)[:120] for i in temporal[:5]]
            crossfield = logical_val.get('cross_field_issues', [])
            if isinstance(crossfield, list) and crossfield:
                validation_block["cross_field"] = [str(i)[:120] for i in crossfield[:5]]
            biz_logic = logical_val.get('business_logic_issues', [])
            if isinstance(biz_logic, list) and biz_logic:
                validation_block["business_logic"] = [str(i)[:120] for i in biz_logic[:5]]
            if validation_block:
                compact["validation_issues"] = validation_block

            # ── 5. CROSS-TABLE ─────────────────────────────────────────────────────
            if isinstance(cross_table, dict) and cross_table:
                ct_block: Dict[str, Any] = {}
                ri_score = cross_table.get('referential_integrity_score')
                if ri_score is not None:
                    ct_block["referential_integrity_score"] = ri_score
                rv = cross_table.get('relationship_violations', [])
                if isinstance(rv, list) and rv:
                    ct_block["violations"] = rv[:3]
                orphans = cross_table.get('orphaned_records', [])
                if isinstance(orphans, list) and orphans:
                    ct_block["orphaned_records"] = orphans[:3]
                if ct_block:
                    compact["cross_table"] = ct_block

            # ── 6. BUSINESS RULE VIOLATIONS ────────────────────────────────────────
            if isinstance(biz_violations, list) and biz_violations:
                compact["top_biz_violations"] = [
                    {k: v for k, v in (bv.items() if isinstance(bv, dict) else {})
                     if k in ('rule', 'table', 'violation_count', 'affected_records')}
                    for bv in biz_violations[:6]
                ]

            # ── PROGRESSIVE TRIM IF STILL TOO LARGE ───────────────────────────────
            result = json.dumps(compact, indent=2, default=str)

            if len(result) > max_chars:
                # Pass 1: strip numeric stats from problem fields
                for tbl in compact["tables"].values():
                    for fd in tbl.get("problem_fields", {}).values():
                        for k in ('min', 'max', 'mean'):
                            fd.pop(k, None)
                result = json.dumps(compact, indent=2, default=str)

            if len(result) > max_chars:
                # Pass 2: shrink anomaly text, drop clean_fields lists
                for tbl in compact["tables"].values():
                    tbl.pop("clean_fields", None)
                    for fd in tbl.get("problem_fields", {}).values():
                        if "anomalies" in fd:
                            fd["anomalies"] = fd["anomalies"][:1]
                        fd.pop("pattern_issues", None)
                result = json.dumps(compact, indent=2, default=str)

            if len(result) > max_chars:
                # Pass 3: keep only summary-level table info, drop field detail
                for tbl in compact["tables"].values():
                    tbl.pop("problem_fields", None)
                result = json.dumps(compact, indent=2, default=str)

            if len(result) > max_chars:
                self.logger.warning(f"Metadata still {len(result)} chars after compression, hard-truncating to {max_chars}")
                result = result[:max_chars] + '\n... [truncated]'

            self.logger.info(
                f"Metadata compressed: {len(table_summaries)} tables → {len(result)} chars "
                f"(limit={max_chars})"
            )
            return result

        except Exception as e:
            self.logger.error(f"Error in _compress_metadata_for_prompt: {e} — falling back to raw truncation")
            import traceback as _tb
            self.logger.debug(_tb.format_exc())
            raw = json.dumps(metadata_summary, indent=2, default=str)
    ...
    def _create_fallback_enrichment(self, metadata_summary: Dict[str, Any], corrected_metrics: Dict[str, Any], logical_validation: Dict[str, Any], cross_table_analysis: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        # Defensive: Ensure all inputs are dicts
        metadata_summary = metadata_summary or {}
        corrected_metrics = corrected_metrics or {"quality_grade": "C", "corrected_score": 50}
        logical_validation = logical_validation or {}
        ...
        if isinstance(metadata_summary, dict):
            dataset_overview = metadata_summary.get('dataset_overview', {})
        table_names = list(dataset_overview.get('table_summaries', {}).keys()) if isinstance(dataset_overview, dict) and 'table_summaries' in dataset_overview else ['unknown_table']
        table_count = len(table_names)
        dataset_type = "multi_table" if table_count > 1 else "single_table"
        
        total_records = dataset_overview.get('total_records', 0) if isinstance(dataset_overview, dict) else 0
        
        production_readiness = "Not Ready"
        if corrected_metrics.get('quality_grade') in ['A', 'B']:
            production_readiness = "Ready"
        elif corrected_metrics.get('quality_grade') == 'C':
            production_readiness = "Needs Review"
        
        overall_risk = "Low"
        if corrected_metrics.get('critical_issues', 0) > 0:
            overall_risk = "Critical"
        elif corrected_metrics.get('major_issues', 0) > 5:
            overall_risk = "High"
        elif corrected_metrics.get('major_issues', 0) > 0:
            overall_risk = "Medium"
        
        table_analysis = []
        for table_name in table_names:
            table_summary = dataset_overview.get('table_summaries', {}).get(table_name, {}) if isinstance(dataset_overview, dict) else {}
            table_analysis.append({
                "table_name": table_name,
                "record_count": table_summary.get('record_count', 0),
                "table_quality_score": corrected_metrics.get('corrected_score', 50),
                "completeness_rate": corrected_metrics.get('component_scores', {}).get('completeness', 50),
                "validity_rate": corrected_metrics.get('component_scores', {}).get('validity', 50),
                "consistency_rate": corrected_metrics.get('component_scores', {}).get('consistency', 50),
                "business_rule_compliance": corrected_metrics.get('component_scores', {}).get('business_rule_compliance', 50),
                "critical_fields": table_summary.get('critical_fields', []),
                "relationship_role": "standalone" if dataset_type == "single_table" else "unknown",
                "key_issues": [
                    f"Automated analysis found {corrected_metrics.get('critical_issues', 0)} critical issues",
                    f"Business rule compliance needs review"
                ]
            })
