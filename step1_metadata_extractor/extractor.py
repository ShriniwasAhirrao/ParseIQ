...
# ===================================================================================================================================================================
    ...
    def _analyze_table_detailed(self, records: List[Dict], table_name: str) -> Dict[str, Any]:
        ...
        # Table-level anomaly: duplicate rows
        dup_count = table_metadata["data_profiling"]["duplicate_analysis"].get("total_duplicates", 0)
        if dup_count > 0:
            deduction = min(dup_count * 2, 20)
            table_metadata["data_quality_score"] = max(0.0, round(table_metadata["data_quality_score"] - deduction, 2))
            table_metadata["anomaly_summary"]["total_anomalies"] += 1
            table_metadata["anomaly_summary"]["anomaly_types"]["DUPLICATE_ROWS_DETECTED"] = dup_count
            dup_msg = f"Table has {dup_count} duplicate row(s)"
            if dup_msg not in table_metadata["top_issues"]:
                table_metadata["top_issues"].insert(0, dup_msg)
    ...
    def extract_metadata(self, data: Union[Dict, List[Dict]]) -> Dict[str, Any]:
        """Main metadata extraction function that determines dataset type and processes accordingly"""
        
        # Determine if this is a multi-table or single-table dataset
        if isinstance(data, dict):
            # Check if this dict contains multiple tables (arrays of objects)
            table_candidates = {}
            
            for key, value in data.items():
                if isinstance(value, list) and len(value) > 0:
                    # Check if list contains dictionaries (table structure)
                    if all(isinstance(item, dict) for item in value[:5]):  # Check first 5 items
                        table_candidates[key] = value
            
            if len(table_candidates) > 1:
                print(f"🗂️  Multi-table dataset detected: {list(table_candidates.keys())}")
                return self._extract_multi_table_metadata(table_candidates)
            elif len(table_candidates) == 1:
                # Single table within a dict wrapper
                table_name, table_data = next(iter(table_candidates.items()))
                print(f"📋 Single table dataset detected: {table_name}")
                return self._extract_single_table_metadata(table_data)
            else:
                # Treat the entire dict as a single record
                print(f"📋 Single record dataset detected")
                return self._extract_single_table_metadata([data])
        
        elif isinstance(data, list):
            print(f"📋 Single table dataset detected (list format)")
            return self._extract_single_table_metadata(data)
...
# ===================================================================================================================================================================
    ...
    def _analyze_cross_table_relationships(self, tables: Dict[str, List[Dict]]) -> Dict[str, Any]:
        ...
        for i, table1_name in enumerate(table_names):
            for j, table2_name in enumerate(table_names[i+1:], i+1):
                table1_attrs = self._collect_all_attributes(tables[table1_name])
                table2_attrs = self._collect_all_attributes(tables[table2_name])
                
                # Look for potential FK relationships
                common_attrs = table1_attrs.intersection(table2_attrs)
                for attr in common_attrs:
                    ...
                    for record in tables[table1_name]:
                        if isinstance(record, dict) and attr in record and record[attr]:
                            table1_values.add(str(record[attr]))
                    
                    for record in tables[table2_name]:
                        if isinstance(record, dict) and attr in record and record[attr]:
                            table2_values.add(str(record[attr]))
                    
                    # Check for potential FK relationship
                    if table1_values and table2_values:
                        overlap = len(table1_values.intersection(table2_values))
                        if overlap > 0:
                            relationships["potential_foreign_keys"].append({
                                ...
                                "table1_unique_values": len(table1_values),
                                "table2_unique_values": len(table2_values)
                            })
    ...
    def _detect_anomalies(self, attr_metadata: Dict[str, Any], values: List[Any]) -> List[str]:
        ...
        # Future date detection — check string columns that contain ISO dates
        if attr_metadata.get("data_type") == "string":
            ...
            for v in values:
                if isinstance(v, str) and re.match(r'^\d{4}-\d{2}-\d{2}', v):
                    try:
                        if datetime.strptime(v[:10], '%Y-%m-%d') > today:
                            future_found = True
                            break
                    except ValueError:
                        pass
            if future_found:
                anomalies.append("FUTURE_DATE_DETECTED")

        # Mixed data types — flag columns where non-null values span incompatible types
        if len(values) > 1:
            raw_types = set()
            for v in values:
                if isinstance(v, bool):
                    ...
                    raw_types.add('dict')
            if len(raw_types) > 1:
                anomalies.append("MIXED_DATA_TYPES")
    ...
    def _calculate_quality_score(self, attributes: Dict[str, Any]) -> float:
        ...
        # Average of per-attribute quality scores
    ...
    def _identify_top_issues(self, attributes: Dict[str, Any]) -> List[str]:
        ...
        for attr_name, attr_data in attributes.items():
            ...
            for anomaly in anomalies:
                ...
                elif anomaly == "FUTURE_DATE_DETECTED":
                    issues.append(f"'{attr_name}' contains future date values")
                elif anomaly == "MIXED_DATA_TYPES":
                    issues.append(f"'{attr_name}' contains mixed data types")
                else:
